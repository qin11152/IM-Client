...
namespace module
{
    ...
    void FileUploader::uploadFileWithJson(const QString& url, const QString& filePath, TaskForResponse task)
    {
        auto self = shared_from_this();
        manager = new QNetworkAccessManager();
        QHttpMultiPart* multiPart = new QHttpMultiPart(QHttpMultiPart::FormDataType);
        ...
        QHttpPart filePart;
        filePart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"file\"; id=\"" + PublicDataManager::get_mutable_instance().getMyId() + "\"; filename = \"" + QFileInfo(filePath).fileName() + "\""));
        QMimeType mimeType = QMimeDatabase().mimeTypeForFile(filePath);
        filePart.setHeader(QNetworkRequest::ContentTypeHeader, QVariant(mimeType.name()));
        QFile* file = new QFile(filePath);
        if (!file->open(QIODevice::ReadOnly)) {
        ...
        connect(reply, &QNetworkReply::finished, this, [this, reply, &loop, file, task]() {
            if (reply->error() == QNetworkReply::NoError) 
            {

                task(std::make_shared<protocol::JsonBaseData>(), reply->readAll().toStdString());
            }
            else 
            {
                emit uploadFinished(false, reply->errorString());
