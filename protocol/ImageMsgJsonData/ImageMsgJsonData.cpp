#include "ImageMsgJsonData.h"

ImageMsgJsonData::ImageMsgJsonData(const std::string& message/*=""*/)
{
    parse(message);
}

void ImageMsgJsonData::parse(const std::string& message)
{
    if (message.empty())
    {
        return;
...
std::string ImageMsgJsonData::generateJson()
{
    StringBuffer strbuf;
